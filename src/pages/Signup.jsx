...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[a-zA-Z0-9-_\/]+$/.test(formData.username)) {
      setError('Username can only contain letters, numbers, hyphens, and slashes.');
      return;
    }
    setError('');
    setIsLoading(true);
  ...
  return (
    <div className="min-h-screen h-[120vh] flex flex-col">
      <div className="sticky top-0 h-[100px] lg:hidden left-0 w-full bg-white shadow-md z-10 flex  p-4 flex-col">
        <img src="/vivstock-purple.png" alt="Welcome" className="w-[90px] -mb-4 -ml-2" />
        <p className="text-[#3d354b] ml-2">vivstock</p>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 text-black">
              ...
              <div>
                <p className="cursor-pointer mt-[-10px] text-gray-500 opacity-[0.7]" onClick={() => setShowReferral(!showReferral)}>
                  Referral link {showReferral ? <FaChevronUp className='relative top-[-18px] left-[90px]' size={12}/> : <FaChevronDown size={12} className='relative top-[-18px] left-[90px]' />}
                </p>
                {showReferral && (
                  <div>
                    <input
